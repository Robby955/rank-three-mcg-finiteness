...
For integers $g \ge 5$ and $n \ge 0$, consider a complex representation

```math
\rho:\pi_1(\Sigma_{g,n})\longrightarrow \mathrm{GL}_3(\mathbb{C}).
```

The manuscript argues that if the conjugacy class of $\rho$ has finite orbit
under the mapping class group, then $\rho$ has finite image.

Landesman and Litt prove finite image when $r < \sqrt{g+1}$. In rank three,
their published theorem starts at $g \ge 9$; the candidate argument treats the
remaining genera $8,7,6,5$.
