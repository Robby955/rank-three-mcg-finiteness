...
- `verify_q10_jet.py` retains the canonical `2 + 1` parahoric bracket and
  moving-line quotient calculation from the earlier review copy. It checks
  the pure and mixed `q = 10` maps, their constant minors, the integral
  involution exchanging the two pure orientations, and moving-frame and
  line-rescaling invariance. This is now a non-load-bearing local diagnostic:
  the manuscript closes `(S, q) = (2, 10)` by the global extension-naturality
  and normalizer argument instead.
