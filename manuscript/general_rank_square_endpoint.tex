...
most $w$, so \cref{cor:fixed-part} gives
\[
 w\ge
 \begin{cases}
 2g-2,&u=1,\\
...
We now prove \cref{thm:square-endpoint}.  The following table records the
...
\cref{cor:artin-endpoint}; Schur and Leray are unchanged\\
...
\[
 r^2-1=g.
\]
Thus \cref{cor:artin-endpoint} replaces Theorem 6.2.1 in both strong
...
\begin{proof}[Proof of \cref{thm:square-endpoint}]
The strict case is published.  At equality, the semisimple case is
\cref{prop:semisimple}.  For an arbitrary representation, apply the
...
\begin{enumerate}
\item the equality case in \cref{prop:endpoint-pairing}, especially the
deduction $N^t=0$, the forced vanishing of every parabolic weight, and the
final equality-of-slopes contradiction;
\item the rank-$g$ endpoint in \cref{cor:artin-endpoint}; and
\item the strict support margin in \cref{lem:extensions}.
\end{enumerate}
...
Putman--Wieland inputs in their published forms.  It neither reproves those
theorems nor claims that an internal reconstruction is a substitute for
specialist review.
