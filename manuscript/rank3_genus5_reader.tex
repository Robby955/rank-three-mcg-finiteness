...
\date{Version 0.1.6-candidate\\25 August 2026}
...
The organizing refinement retains their rigidity, integrality, and
deformation framework while replacing the uniform adjoint-rank input by the
classification of projective Zariski closures in rank three.  The only
adjoint constituents not already covered by the published strict-range
theorem have ranks eight, six, and five: the dense adjoint constituent, the
$S_3$-monomial constituent, and the principal-$\PGL_2$ constituent.  An
equality-case refinement of the Landesman--Litt estimate,
Harder--Narasimhan analysis, and a normalizer boundary-cocycle obstruction
eliminate the remaining cases.  Separate
...
The organizing idea is to retain the rigidity, integrality, and deformation
framework of Landesman--Litt while replacing its uniform adjoint-rank input,
in rank three, by the classification of projective Zariski closures.  The
only resulting adjoint constituents not already covered by their strict-range
theorem have ranks eight, six, and five: the dense adjoint constituent, the
$S_3$-monomial constituent, and the principal-$\PGL_2$ constituent.  The new
work is therefore concentrated in these cases and in the equality interface
needed to re-enter the published propagation pipeline.
...
OpenAI Codex (GPT-5.6) was used during proof search, drafting, and internal
error checking, including exploration and testing of the
Harder--Narasimhan branch eliminations, the normalizer boundary-cocycle
argument, and the formal-propagation interfaces.  The author formulated the
research direction and rank-three projective-closure reduction, selected and
revised the arguments, and is responsible for all statements, proofs,
