...
\date{Version 0.1.2-candidate\\10 August 2026}
...
Here $F$ has rank eight and $2g-2=10$.  Let $N\subset A$ be the HN part of
slope $>10$, and write $t=\rk N$.  The quotient-rank estimate gives
$t\le2$.
...
\[
 ...
 \qquad \deg(F/A)=10,
\]
and hence $\deg M=0$, contrary to parabolic stability.  If $t=1$, the two
integral degree offsets left by the quotient-slope and Clifford inequalities
are the equality offset, which again gives $\deg M=0$, and one strict offset.
In the strict offset the dual low-slope quotient pulls back to a proper
ordinary degree-zero subbundle of $E$ or, at a puncture, to a quotient whose
induced weight violates strict parabolic stability.  Thus no high-HN branch
survives.
...
\]
For $t=1$, the HN upper degree and Clifford inequalities give
$q+y\ge16$ and $q\le y+4$, where $y=x-S$.  The case $y=5$ is numerically
impossible; $y=8$ gives a degree-zero subbundle; and $y=6,7$ violate the
parabolic quotient-weight inequality.  Thus \eqref{eq:highres} is the only
high-HN family.
...
In the $2+1$ contracted fibre, the only quotient-bracket ranks at most two are
a pure radical line and an isotropic mixed radical line.  For an arbitrary
first-order correction, the induced map
\[
 T:\ker B_0\longrightarrow\coker B_0
\]
has rank at least two in the pure case and at least three in the mixed case.
Indeed, in suitable bases the respective matrices contain constant minors
\[
 \det T_{\{1,3\},\{5,6\}}=1,
 \qquad
 \det T_{\{1,2,3\},\{3,4,5\}}=1.
\]
