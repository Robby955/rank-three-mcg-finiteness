...
\date{Version 0.1.3-candidate\\Expanded review copy\\10 August 2026}
...
\section{The rank-eight coefficient in genera seven and six}
...
$h^0(Q\otimes M^{-1})\le1$ on a nonhyperelliptic fibre.  The inverse image
$P\subset E$ has $H^0(P)=0$ by stability.  Proposition~\ref{prop:adjoint-universal},
...
nonzero.  On a nonhyperelliptic genus-six curve, a degree-two line has at
...
\section{Genus five: monomial coefficient}
...
For $t=3$, \eqref{eq:cliff} gives $q\le8$.  For $t=2$, the offset $16$
produces a proper degree-zero rank-six subbundle of $E$, while offset $15$
forces
\[
...
\end{array}
\]
The row $y=8$ contradicts parabolic stability because $P$ has degree zero
and nonnegative induced weights.  For $y=6,7$, let $\beta$ be the total
...
\section{Zero-weight genus-five branches}
...
\[
 V\simeq\Ocal_C^6.
\]
Since $h^0(L)\le1$ on a nonhyperelliptic genus-five curve, all entries of
...
\section{Punctured no-high branches}
...
\]
On a very general pointed genus-five fibre, each indicated section space is
...
\[
 L\simeq\Ocal_C(3p).
\]
Choose the very general marked fibre so that $p$ is non-Weierstrass; then
...
at least two on a hyperplane.  Thus $L$ is effective.  Its unique degree-two
divisor can be supported only at the sole nonordinary point:
\[
 L\simeq\Ocal_C(2p),
 \qquad D\simeq\Ocal_C.
\]
The six global bracket sections then have length-two jets spanning at most
one complex dimension.

Let $W(z)$ be the local rank-six subbundle.  A one-dimensional jet image gives
a five-dimensional kernel in the constant section space.  Because the global
subbundle is trivial, evaluation identifies it with a five-dimensional
subspace of $W_0$.  For a moving vector $v(z)=v_0+zv_1$, the vanishing jet
equation is $B_0v_0=0$ and $B_1v_0+B_0v_1=0$; projection to
$\coker B_0$ removes the moving-frame term $B_0v_1$.  Hence the induced map
$T:\ker B_0\to\coker B_0$ must vanish on a five-dimensional subspace.

We now display the finite local calculation.  Put $R=\C[[z]]$ and use the
canonical $2+1$ lattice frame
\[
 (e_1,\ldots,e_8)
 =(H_1,H_2,E_{12},E_{21},zE_{13},zE_{23},E_{31},E_{32}),
\]
where $H_1=\operatorname{diag}(1,-1,0)$ and
$H_2=\operatorname{diag}(0,1,-1)$.  The special fibre is
$\mathfrak{gl}_2\ltimes(V\oplus V^\vee)$.  A line with nonzero Levi part
has quotient-bracket rank at least three.  For a radical vector $(u,v)$,
the ranks at most two are exactly the pure case, in which one of $u,v$
vanishes, and the mixed isotropic case $u,v\ne0$, $v(u)=0$.

In the pure normal form take
\[
 m=e_5+z\sum_{i=1}^8a_i e_i+O(z^2).
\]
On $G_0=\mathfrak p_R/\langle e_5\rangle$, the constant bracket has
$B_0(e_4)=-e_6$ and vanishes on the other quotient basis vectors.  With
ordered bases
\[
 \ker B_0=(e_1,e_2,e_3,e_6,e_7,e_8),
 \qquad
 \coker B_0=(e_1,e_2,e_3,e_4,e_7,e_8),
\]
...
a_1&a_1&-a_4&0&1&0\\
a_2&a_2&0&0&1&0\\
-a_3&2a_3&2a_1-a_2&0&0&1\\
3a_4&0&0&0&0&0\\
2a_7&2a_7&0&0&-a_1-a_2&-a_4\\
0&3a_8&a_7&0&-a_3&a_1-2a_2
\end{pmatrix}.
\]
\endgroup
The $R$-linear Lie-lattice involution
\[
 X\longmapsto-DX^{\mathsf t}D^{-1},
 \qquad D=\operatorname{diag}(z,z,1),
\]
exchanges $e_5$ with $-e_7$ and $e_6$ with $-e_8$, so this calculation
covers both pure radical orbits, including arbitrary first-order
corrections.

In the mixed isotropic normal form take
\[
 m=e_5+e_8+z\sum_{i=1}^8a_i e_i+O(z^2).
\]
Here $B_0(e_2)=3e_8$ and $B_0(e_4)=-e_6+e_7$.  Use
\[
 \ker B_0=(e_1,e_3,e_6,e_7,e_8),
 \qquad
 \coker B_0=(e_1,e_2,e_3,e_4,e_6),
\]
...
-a_3&2a_1-a_2&0&0&1\\
3a_4&0&0&0&0\\
2a_6+2a_7&0&-a_1+2a_2&-a_1-a_2&-a_4
\end{pmatrix}.
\]
\endgroup
The respective constant submatrices are
\[
 (T_{\mathrm{pure}})_{\{1,3\},\{5,6\}}
 =\begin{pmatrix}1&0\\0&1\end{pmatrix},
 \qquad
 (T_{\mathrm{mixed}})_{\{1,2,3\},\{3,4,5\}}
 =\begin{pmatrix}0&1&0\\-1&1&0\\0&0&1\end{pmatrix}.
\]
Both have determinant one.  Changing the moving frame replaces
$B_1$ by $B_1+B_0U-VB_0$, and rescaling the line adds a multiple of $B_0$;
neither operation changes the induced map
$T:\ker B_0\to\coker B_0$.  Thus $T$ has rank at least two in the pure
case and at least three in the mixed case for every first-order correction.
A one-dimensional two-jet image would force $T$ to vanish on a
five-dimensional subspace, impossible.
...
Let $P\subset E$ be the inverse image of $W=\Ocal_C^5$.  Parabolic
stability gives $H^0(P)=0$, and the branch data give
...
obstruction, and the formal-propagation interfaces have each been re-derived
within the project.  The spectral-projector closure and periodic-chain
determinant theorem are retained as separate reconstructions,
not as load-bearing inputs.  The remaining external task is specialist
verification of the assembled use of the Landesman--Litt Hodge, parabolic,
and deformation machinery.  Exact scripts used during development verify
only finite arithmetic and fibrewise linear algebra; they are not substitutes
for those geometric inputs.
...
successive versions for omitted cases and mismatched hypotheses.  The author
directed their use and is responsible for the manuscript and any errors.
These tools were not treated as mathematical authorities, and the finite
scripts do not verify the imported geometric inputs.
